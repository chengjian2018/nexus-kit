...
repair_confirm_addr = BaseNode(
    ...
    name="地址核对",
    description="复述订单地址，请客户核对是否一致（师傅按此地址上门）",
    task_description="核对上门维修地址是否一致，一致则进入时间协商",
    slots={
        "address_confirmed": "地址是否一致（是/否）",
        "address": "客户口径的维修地址（不一致时记录）",
    },
    # Repair scenario has no arrival step: address confirmed goes straight to time negotiation
...
repair_confirm_time = BaseNode(
    ...
    # Repair-scenario key difference: after confirmation → fault-info collection (not a direct close)
