...
- 拿不准 → 默认 **agent**（框架默认值，`nexus/model/pattern.py:36`），
  但必须说明 fsm 信号为什么不占上风。
- 需求含糊不停摆：套用最安全的默认值，记录你补上的每一条假设，把
  待确认问题列进最终报告——先推断后提案，绝不反复追问。
...
2. **全节点清单。** 每个节点的 `code / name / 用途 / sub_nodes / is_end`，
   ...
   （`plugins={"loop": ...}`）。从满足需求的最小图开始——主干之外每个
   节点都要指出需求信号依据（信号→拓扑映射表：`references/pitfalls.md`
   §11）。
...
2. 新增 `tests/test_<app>_route.py`，离线、不联网：结构断言
   ...
   executor 接线端到端正确。
3. 如实汇报：过了什么、什么是桩、还剩什么。注明真实试跑仍要盯什么——
   收敛轮数、修复成功率、tool 错误率：fake-provider 走查全绿只证明接线
   正确，不证明语义质量。
