...
`get_pattern_custom_config("<code>")` + config.yaml `pattern:` key. Checklist
in `references/template-index.md`.
...
|---|---|
| Known list of independent items, one combined answer at the end | Fan-out `Send` workers + one merge node (repo exemplar: `apps/deep_research_agent`) |
...
| Unknown count, stop on quality or budget | Loop with a semantic stop (stale-N dry exit) + `max_steps` budget (§3) — never an unguarded cycle (repo exemplar: `apps/archify_agent`) |
