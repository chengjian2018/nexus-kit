...
- Assembly view = runtime view: ``nexus.pipeline.resolve_stage_code`` /
  ...
  ``chat._resolve_node_executor_code``'s read order
...
class TemplateInfo:
    ...
    entry_node_code: str
    node_codes: List[str]
    meta: Dict[str, str]            # meta line key/values (literal keys: 业务形态/图类型/借用来源/来源)
    yaml_block: str                 # marker fenced block verbatim ("" = absent)
    cards: Dict[str, int]           # literal keys {"插件卡": n, "工具卡": m} (plugin/tool card counts)
...
def _parse_template_meta(text: str) -> Dict[str, str]:
    """Meta blockquote line -> key/value dict (segments split by the literal ｜)."""
...
def resolved_executor(node, pattern) -> Tuple[str, str]:
    """(code, source layer) — mirrors chat._resolve_node_executor_code:
    declared layers node.plugins['loop'] > pattern.plugins['loop'], falling
    back to default_loop (AGENT nodes)."""
